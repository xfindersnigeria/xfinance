...
9. If migrating an existing deployment onto Goal 25 (tax/email settings, POS,
   standard accounts), run from `apps/api/`, **in this order** (both idempotent):
   `npm run backfill:sales-setup` (COGS 5140 account + default tax settings per
   entity), then `npm run backfill:standard-accounts` (the client's 44 standard
   accounts + 5280 subcategory; skips accounts an entity already has).
...
- [x] **Goal 25** — Delivery tidy-ups (2026-09-18). Full reference: `notes/2026-09-18-tax-email-pos-orders.md`. Migrations `20260919000000_tax_email_orders_free_text`, `20260919010000_store_item_service_price`; backfills `backfill:sales-setup` then `backfill:standard-accounts` (both idempotent).
  - **Settings → Tax** (`src/settings/tax/`): old-UI parity — config switches (taxCalculation/taxInclusive/compoundTax/reverseChargeVat on `Settings`), TaxRate/TaxGroup(+TaxGroupRate, ordered)/TaxExemption/TaxJurisdiction CRUD. One default rate (mirrored to `Settings.taxRate`). All tax maths in `src/sales/sales-tax.util.ts` (`resolveDocumentTax`, `computeDocumentTotals`, `effectiveGroupRate`, `splitNetRevenue`); documents store `taxRate`/`taxName`/`taxInclusive`; invoice + receipt posting credit revenue with net (total − tax) so inclusive pricing balances. Frontend: shared `components/local/shared/TaxSelect.tsx` (+ `useDefaultTax`) on invoice/receipt/bill/POS forms, `lib/tax/totals.ts` mirrors the server.
  - **Settings → Email** (`src/settings/email/`, `src/email/`): per-entity SMTP (`EmailSettings`, password AES-256-GCM via `SMTP_ENCRYPTION_KEY`/`COOKIE_SECRET`) else platform ZeptoMail on the entity's behalf with Reply-To (`EntityMailerService`); editable templates (`EmailTemplate`, defaults in `email-templates.ts`); `DocumentEmailService` sends invoice/payment-confirmation/receipt/reminder/statement; `EmailSchedulerService` crons (reminders 08:00 daily, statements 07:00 on the 1st) claim `InvoiceReminderLog`/`CustomerStatementLog` rows before sending — register the scheduler in EmailSettingsModule ONLY. `POST sales/invoices/:id/send` takes `{to?}`; `POST sales/receipts/:id/email {to}`.
  - **POS / Orders / Online store** (`src/product/orders/`): `Order`/`OrderItem`; `POST orders/pos-checkout` = guarded stock decrement + InventoryMovement + Completed receipt (lines carry `ReceiptItem.storeItemId`/`costPrice`) + order in one transaction, then the existing receipt posting job (now also Dr COGS 5140-01 / Cr Inventory 1130-01). Online: `Entity.storeSlug`/`onlineStoreEnabled`, public `GET/POST public/store/:slug[/orders]` (tenant-scoped, per-IP rate limit) → Pending order; `PATCH orders/:id/complete` (claimed with guarded updateMany) runs the same fulfilment; `/cancel`. Store items gained GET/PUT/DELETE `:id` and images.
  - **Typed-in customers/vendors on invoices & bills**: `Invoice.customerId`, `Bills.vendorId`, `PaymentMade.vendorId` optional (SetNull) + `customerName`/`customerEmail`/`vendorName`; `src/sales/party.util.ts` presents typed-in parties in the saved-record shape; reports group them by name (`partyOf`). Deleting a customer stamps its name on its documents instead of deleting invoices.
  - **Standard accounts**: `seeders/seed-standard-entity-accounts.ts` (44 client accounts, new 5280 G&A subcategory) runs BEFORE `seedDefaultEntityAccounts` in the entity job (default seeder skips covered subcategories); skips by code or name.
  - **Fixed**: bill tax % sent as an amount; bill posting ignored discount (AP ≠ bill total); bills list ignored `vendorId`; store-items edit/delete endpoints missing; service item price saved ×100; group Create Account showed a group picker inside a group and only 10 entities. Report PDFs always landscape, A3 when > 10 columns.
