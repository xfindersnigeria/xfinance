...
function row90Plus(r: AgedReceivablesRow) { return r.days91_120 + r.days120Plus; }

function rowByKey(r: AgedReceivablesRow, key: BucketKey): number {
...
function riskBadge(row: AgedReceivablesRow): { label: string; cls: string } {
  const overdue90 = row90Plus(row);
  const overdue60 = row.days61_90 + overdue90;
  if (overdue60 > 0) return { label: "High",  cls: "bg-red-100 text-red-700" };
  if (row.days1_30 > 0 || row.days31_60 > 0) return { label: "Low", cls: "bg-amber-100 text-amber-700" };
  return { label: "Good", cls: "bg-green-100 text-green-700" };
}
...
export default function AgedReceivables() {
  ...
  // Aging distribution vertical bar chart data
  ...
  // 61+ days customers stacked chart
  const overdueCustomers = useMemo(() =>
    rows
      .filter(r => r.days61_90 + row90Plus(r) > 0)
      .slice(0, 8)
      .map(r => ({
        name: r.customerName.length > 16 ? r.customerName.slice(0, 16) + "…" : r.customerName,
  ...
  // Critical 90+ customers
  const critical90Customers = useMemo(() =>
    rows
  ...
  const creditOverLimit = rows.filter(r => {
    const risk = riskBadge(r);
    ...
    <div className="flex flex-col gap-6 pb-12">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-semibold text-slate-900">Aged Receivables Report</h1>
          <p className="text-sm text-slate-500 mt-0.5">Track outstanding receivables by aging period</p>
        </div>
        <div className="mt-7">
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-col gap-1">
              ...
              <p className="text-xs text-slate-400">All receivables</p>
            </div>
            {/* Bucket cards */}
            {BUCKETS.map(b => {
          ...
          {grandTotal > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Aging Distribution — vertical bars */}
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={agingBarData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                    ...
                    <Tooltip
                      {...tooltipStyle}
                      formatter={(v: number) => [fmtShort(v, sym), "Amount"]}
                    />
          ...
          {/* Customers with Aged Balances (61+ Days) */}
          {overdueCustomers.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={overdueCustomers} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                  <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#64748b" }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 10, fill: "#94a3b8" }} tickFormatter={v => fmtAxis(v, sym)} axisLine={false} tickLine={false} width={60} />
                  <Tooltip
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <p className="font-semibold text-slate-900">Customer Aging Details</p>
            </div>
              ...
              <table className="w-full min-w-[860px]">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="px-5 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wide">Risk</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.length === 0 ? (
                    ...
                    <>
                      {rows.map((row: AgedReceivablesRow) => {
                        ...
                        const risk = riskBadge(row);
                        return (
                          <tr key={row.customerName} className="border-t border-slate-50 hover:bg-slate-50/40">
                            ...
                            <td className="px-5 py-3 text-center">
                              <span className={cn("text-xs font-semibold px-2.5 py-1 rounded-full", risk.cls)}>
                                {risk.label}
                              </span>
                            </td>
                          </tr>
          ...
          {total90Plus > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="font-semibold text-red-800">Critical Collection Alert</p>
              </div>
              <p className="text-sm text-red-700 ml-7 mb-3">
                You have <strong>{fmtShort(total90Plus, sym)}</strong> in receivables aged over 90 days.
                These accounts require immediate attention as collection likelihood decreases significantly after 90 days.
              </p>
              <ul className="ml-7 flex flex-col gap-1">
                {critical90Customers.map(r => (
                  <li key={r.customerName} className="text-sm text-red-700">
                    · <strong>{r.customerName}</strong>: {fmtShort(row90Plus(r), sym)}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {grandTotal > 0 && (
            <div className="bg-slate-800 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="font-semibold text-white">Collection Insights</p>
              </div>
              <ul className="flex flex-col gap-2 ml-7">
                <li className="text-sm text-slate-300">
                  <strong className="text-teal-400">{currentPct}%</strong>{" "}
                  of receivables are current
                  {parseFloat(currentPct) >= 70
                    ? ", which is a healthy collection rate."
                    : " — consider accelerating collection efforts."}
                </li>
                {overdue60Plus > 0 && (
                  <li className="text-sm text-slate-300">
                    Focus collection efforts on the{" "}
                    <strong className="text-amber-400">{fmtShort(overdue60Plus, sym)}</strong>{" "}
                    in receivables aged over 60 days to minimize bad debt exposure.
                  </li>
                )}
                {creditOverLimit > 0 && (
                  <li className="text-sm text-slate-300">
                    <strong className="text-amber-400">{creditOverLimit}</strong>{" "}
                    customer{creditOverLimit !== 1 ? "s" : ""} with high risk aging.
                    Consider reviewing credit terms.
                  </li>
                )}
              </ul>
            </div>
