...
const BUCKETS = [
  { key: "current", label: "Current", color: "#10b981" }, { key: "days1_30", label: "1–30 days", color: "#f59e0b" },
  { key: "days31_60", label: "31–60 days", color: "#f97316" }, { key: "days61_90", label: "61–90 days", color: "#ef4444" },
  { key: "days91_120", label: "91–120 days", color: "#dc2626" }, { key: "days120Plus", label: "120+ days", color: "#7f1d1d" },
] as const;

const BUCKET_CELL_CLS: Record<string, string> = {
  current: "text-slate-700", days1_30: "text-amber-600", days31_60: "text-orange-600",
  days61_90: "text-red-600", days91_120: "text-red-700", days120Plus: "text-red-800 font-semibold",
};
...
export default function AgedPayables() {
  ...
  const overduePct = grandTotal > 0 ? Math.round(((grandTotal - (totals?.current ?? 0)) / grandTotal) * 100) : 0;

  const chartData = useMemo(() =>
    rows.slice(0, 10).map(r => ({
      name: r.vendorName.length > 20 ? r.vendorName.slice(0, 20) + "…" : r.vendorName,
      Current: r.current, "1–30": r.days1_30, "31–60": r.days31_60,
      "61–90": r.days61_90, "91–120": r.days91_120, "120+": r.days120Plus,
    })), [rows]);
    ...
    <div className="flex flex-col gap-6 pb-12">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-semibold text-slate-900">Aged Payables</h1>
          <p className="text-sm text-slate-500 mt-0.5">Vendor balances by aging bucket</p>
        </div>
      ...
      {isLoading && (
        <div className="flex flex-col gap-4">
          ...
          <Skeleton className="h-16 rounded-2xl" /><Skeleton className="h-72 rounded-2xl" /><Skeleton className="h-80 rounded-2xl" />
        </div>
      )}
      {!isLoading && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <KPICard label="Total Outstanding" value={fmtShort(grandTotal, sym)} sub={`${rows.length} vendors`} />
          ...
          {grandTotal > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="flex h-5 rounded-lg overflow-hidden gap-px">
                {BUCKETS.map(b => { const val = (totals as any)?.[b.key] ?? 0; const pct = grandTotal > 0 ? (val / grandTotal) * 100 : 0; return pct > 0 ? <div key={b.key} style={{ width: `${pct}%`, backgroundColor: b.color }} title={`${b.label}: ${fmtShort(val, sym)}`} /> : null; })}
              </div>
              <div className="flex flex-wrap gap-4 mt-3">
                {BUCKETS.map(b => { const val = (totals as any)?.[b.key] ?? 0; return (<div key={b.key} className="flex items-center gap-1.5 text-xs text-slate-600"><span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: b.color }} />{b.label} — {grandTotal > 0 ? ((val / grandTotal) * 100).toFixed(1) : "0.0"}%</div>); })}
              </div>
            </div>
          )}
          ...
          {chartData.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <p className="font-semibold text-slate-800 mb-4">Top Vendors by Aging</p>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart layout="vertical" data={chartData} margin={{ top: 4, right: 24, left: 8, bottom: 4 }}>
                  ...
                  {BUCKETS.map(b => <Bar key={b.key} dataKey={b.label} stackId="a" fill={b.color} maxBarSize={14} />)}
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100"><p className="font-semibold text-slate-900">Aged Payables Detail</p></div>
              ...
              <table className="w-full min-w-[820px]">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    {BUCKETS.map(b => <th key={b.key} className="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{b.label}</th>)}
                    <th className="px-5 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.length === 0
                    ? <tr><td colSpan={8} className="px-5 py-16 text-center text-slate-400 text-sm">No aged payables found.</td></tr>
                    : <>
                        {rows.map((row: AgedPayablesRow) => (
                          <tr key={row.vendorName} className="border-t border-slate-50 hover:bg-slate-50/60">
                        ...
                        {totals && (
                          <tr className="border-t-2 border-slate-200 bg-slate-800">
                            ...
                            {BUCKETS.map(b => <td key={b.key} className="px-4 py-3 text-right text-sm font-bold text-slate-200">{(totals as any)[b.key] > 0 ? fmtShort((totals as any)[b.key], sym) : "—"}</td>)}
