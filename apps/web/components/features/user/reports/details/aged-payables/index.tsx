...
export default function AgedPayables() {
  ...
  const { data: rawData, isLoading } = useAgedPayables({ asOfDate: endDate });
    ...
    <div className="flex flex-col gap-4 pb-10">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-xl font-semibold">Aged Payables</h1>
          <p className="text-sm text-primary">Vendor balances by aging bucket</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px]">
              ...
              <tbody>
                {data?.rows?.length ? data.rows.map((row: AgedPayablesRow) => (
                  <tr key={row.vendorName} className="hover:bg-gray-50 border-t border-gray-50">
                  ...
                  <tr><td colSpan={8} className="px-4 py-16 text-center text-gray-400 text-sm">No aged payables data found.</td></tr>
                )}
                {totals && (
