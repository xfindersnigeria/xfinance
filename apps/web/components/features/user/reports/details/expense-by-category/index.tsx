      ...
      <tr className="hover:bg-gray-50 border-t border-gray-100 cursor-pointer" onClick={() => setExpanded((v) => !v)}>
        ...
        <td className="px-4 py-2.5">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: CHART_COLORS[colorIdx % CHART_COLORS.length] }} />
            <span className="text-sm font-medium text-gray-800">{row.categoryName}</span>
          </div>
        </td>
        <td className="px-4 py-2.5 text-right text-sm font-semibold">{fmt(row.total, sym)}</td>
        <td className="px-4 py-2.5 text-right text-sm text-gray-500">{row.percentOfTotal.toFixed(1)}%</td>
      </tr>
      {expanded && row.accounts.map((acc) => (
        <tr key={acc.accountId} className="bg-gray-50 border-t border-gray-100">
          <td />
          <td className="px-4 py-1.5 pl-10 text-xs text-gray-600">{acc.accountCode} — {acc.accountName}</td>
          <td className="px-4 py-1.5 text-right text-xs">{fmt(acc.amount, sym)}</td>
          <td className="px-4 py-1.5 text-right text-xs text-gray-400">{acc.percentOfCategory.toFixed(1)}%</td>
        </tr>
      ))}
    </>
  );
...
export default function ExpenseByCategory() {
  ...
  const pieData = (data?.rows ?? []).map((r) => ({ name: r.categoryName, value: r.total }));
    ...
    <div className="flex flex-col gap-4 pb-10">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-primary">Expenditure breakdown by account category</p>
        </div>
            ...
            <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {data?.rows?.length ? data.rows.map((row: ExpenseByCategoryRow, i) => (
                      <CategoryRow key={row.categoryCode} row={row} sym={sym} colorIdx={i} />
                    )) : (
