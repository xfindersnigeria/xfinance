...
export default function VendorBalances() {
  ...
  const { data: rawData, isLoading } = useVendorBalances({ asOfDate: endDate });
    ...
    <div className="flex flex-col gap-4 pb-10">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-primary">Outstanding balances grouped by vendor</p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <KPICard label="Total Paid" value={fmt(data?.totalPaid ?? 0, sym)} cls="text-green-700" />
              ...
              <table className="w-full min-w-[500px]">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right text-[10px] font-semibold text-gray-500 uppercase">Total Billed</th>
                    <th className="px-4 py-3 text-right text-[10px] font-semibold text-gray-500 uppercase">Total Paid</th>
                    <th className="px-4 py-3 text-right text-[10px] font-semibold text-gray-500 uppercase">Balance</th>
                  </tr>
                </thead>
                <tbody>
                  {data?.rows?.length ? data.rows.map((row: VendorBalanceRow) => (
                    <tr key={row.vendorId} className="hover:bg-gray-50 border-t border-gray-50">
                      ...
                      <td className="px-4 py-2.5 text-right text-sm">{fmt(row.totalBilled, sym)}</td>
                      <td className="px-4 py-2.5 text-right text-sm text-green-700">{fmt(row.totalPaid, sym)}</td>
                      <td className={cn("px-4 py-2.5 text-right text-sm font-medium", row.balance > 0 ? "text-red-600" : "text-gray-800")}>
                        {fmt(row.balance, sym)}
                      </td>
                    </tr>
                  )) : (
                    <tr><td colSpan={4} className="px-4 py-16 text-center text-gray-400 text-sm">No vendor balance data found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
