...
export default function CustomerBalances() {
  ...
  const { data: rawData, isLoading } = useCustomerBalances({ asOfDate });
  const data: CustomerBalancesData | null = (rawData as any)?.data ?? null;
  const rows = data?.rows ?? [];
  const top10 = rows.slice(0, 10).map(r => ({ name: r.customerName.length > 18 ? r.customerName.slice(0, 18) + "…" : r.customerName, Billed: r.totalInvoiced, Received: r.totalReceived }));
    ...
    <div className="flex flex-col gap-6 pb-12">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Outstanding balances grouped by customer</p>
        </div>
      ...
      {!isLoading && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <KPICard label="Total Invoiced" value={fmtShort(data?.totalInvoiced ?? 0, sym)} />
            <KPICard label="Total Received" value={fmtShort(data?.totalReceived ?? 0, sym)} valueClassName="text-green-600 font-bold text-2xl" />
            <KPICard label="Total Balance" value={fmtShort(data?.totalBalance ?? 0, sym)} valueClassName="text-red-500 font-bold text-2xl" />
          </div>

          {top10.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <p className="font-semibold text-slate-800 mb-4">Top Customers</p>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart layout="vertical" data={top10} margin={{ top: 4, right: 24, left: 8, bottom: 4 }}>
                  ...
                  <Bar dataKey="Received" fill="#10b981" radius={[0, 4, 4, 0]} maxBarSize={12} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100"><p className="font-semibold text-slate-900">Customer Balances</p></div>
              ...
              <table className="w-full min-w-[540px]">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="px-5 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide">Total Invoiced</th>
                    <th className="px-5 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide">Total Received</th>
                    <th className="px-5 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide">Balance</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.length === 0
                    ? <tr><td colSpan={4} className="px-5 py-16 text-center text-slate-400 text-sm">No customer balance data found.</td></tr>
                    : <>
                        {rows.map((row: CustomerBalanceRow) => (
                          <tr key={row.customerId} className="border-t border-slate-50 hover:bg-slate-50/60">
                            ...
                            <td className="px-5 py-3 text-right text-sm text-slate-700">{fmtShort(row.totalInvoiced, sym)}</td>
                            <td className="px-5 py-3 text-right text-sm text-green-600">{fmtShort(row.totalReceived, sym)}</td>
                            <td className={cn("px-5 py-3 text-right text-sm font-medium", row.balance > 0 ? "text-red-500" : "text-slate-400")}>{fmtShort(row.balance, sym)}</td>
                          </tr>
                        ))}
                        <tr className="border-t-2 border-slate-200 bg-slate-50/80">
                          <td className="px-5 py-3 text-sm font-semibold text-slate-900">Total</td>
                          <td className="px-5 py-3 text-right text-sm font-bold text-slate-900">{fmtShort(data?.totalInvoiced ?? 0, sym)}</td>
                          <td className="px-5 py-3 text-right text-sm font-bold text-green-600">{fmtShort(data?.totalReceived ?? 0, sym)}</td>
                          <td className="px-5 py-3 text-right text-sm font-bold text-red-500">{fmtShort(data?.totalBalance ?? 0, sym)}</td>
                        </tr>
                      </>
                  }
                </tbody>
