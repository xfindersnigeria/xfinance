  ...
  return (
    <>
      <tr className="border-t border-slate-50 hover:bg-slate-50/60 cursor-pointer" onClick={() => setExpanded(v => !v)}>
        ...
        <td className="px-4 py-3 text-sm font-medium text-slate-800">{row.departmentName}</td>
...
export default function SuppliesConsumptionByDepartment() {
  ...
  const buildExport = (): ReportExportPayload | null => {
    if (!data) return null;
    return {
      title: "Supplies Consumption by Department",
      scope: "entity",
      ...
      summary: [
        ...
        { label: "Departments", value: data.summary.departmentCount, format: "number" },
      ],
      columns: [
        { key: "name", label: "Department / Supply" },
      ...
      sections: [
        {
          rows: [
            ...data.rows.flatMap((r) => [
              { kind: "header" as const, cells: { name: r.departmentName, qty: r.totalQuantity, value: r.totalValue } },
    ...
    <div className="flex flex-col gap-6 pb-12">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-semibold text-slate-900">Supplies Consumption by Department</h1>
          <p className="text-sm text-slate-500 mt-0.5">Supply usage and cost grouped by department</p>
        </div>
        <div className="mt-7">
      ...
      {!isLoading && (
        <>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100"><p className="font-semibold text-slate-900">Consumption by Department</p></div>
