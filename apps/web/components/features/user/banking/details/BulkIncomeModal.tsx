...
import { useBulkImportReceipts } from "@/lib/api/hooks/useSales";
...
const TARGET_FIELDS = [
  ...
  { key: "amount", label: "Amount", required: true, hint: "Income amount (positive)" },
  { key: "reference", label: "Reference", required: false, hint: "Invoice or ref number" },
  { key: "customerName", label: "Customer", required: false, hint: "Payer / customer name" },
] as const;
...
function autoMatch(col: string): TargetKey | "" {
  ...
  if (["customer", "payer", "client", "payee", "party"].some((k) => c.includes(k))) return "customerName";
  return "";
}
...
const CSV_TEMPLATE = `date,description,reference,customerName,amount\n2025-11-24,Consulting fee,INV-2025-014,Acme Corp,85000\n2025-11-23,Product sale,INV-2025-013,Jane Doe,2400\n2025-11-22,Service retainer,INV-2025-012,Beta LLC,1850`;
...
function downloadTemplate() {
  ...
  a.href = url;
  a.download = "bulk_income_template.csv";
  a.click();
...
interface BulkIncomeModalProps {
  open: boolean;
...
export default function BulkIncomeModal({
  ...
  const [mapping, setMapping] = useState<Record<TargetKey, string>>({ date: "", description: "", amount: "", reference: "", customerName: "" });
  const [rows, setRows] = useState<ParsedRow[]>([]);
  const [importResult, setImportResult] = useState<{ imported: number; total: number } | null>(null);

  const bulkImport = useBulkImportReceipts();
  ...
  function reset() {
    ...
    setHeaders([]);
    setSheetRows([]);
    setMapping({ date: "", description: "", amount: "", reference: "", customerName: "" });
    setRows([]);
  ...
  async function handleFile(f: File) {
    ...
    try {
      ...
      setHeaders(h);
      setSheetRows(r);
      const auto: Record<TargetKey, string> = { date: "", description: "", amount: "", reference: "", customerName: "" };
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl lg:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold">Bulk Income Upload</DialogTitle>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border bg-gray-50 p-4">
              ...
              <p className="text-xs font-mono text-gray-500 leading-relaxed">
                date, description, reference, customerName, amount<br />
                <span className="text-gray-400">2025-11-24, Consulting fee, INV-2025-014, Acme Corp, 85000</span>
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl border overflow-hidden">
              <div className="overflow-x-auto">
                <div className="grid grid-cols-[2rem_1fr_1fr_6rem_2rem] min-w-125 text-xs font-semibold text-gray-500 bg-gray-50 px-3 py-2 gap-2 border-b">
                  <span>#</span>
                ...
                <div className="divide-y max-h-64 overflow-y-auto">
                  {rows.map((r, i) => (
                    <div key={i} className="grid grid-cols-[2rem_1fr_1fr_6rem_2rem] min-w-125 items-center px-3 py-2.5 gap-2 hover:bg-gray-50">
                      ...
                      <span className="text-xs text-gray-600 truncate">{r.customerName || "—"}</span>
                      <span className={`text-xs font-semibold ${r.amount > 0 ? "text-green-600" : "text-gray-400"}`}>
            ...
            <div className="flex justify-between items-center pt-1">
              ...
              >
                ...
                {bulkImport.isPending ? "Importing..." : `Import ${rows.length} Income Receipt${rows.length !== 1 ? "s" : ""}`}
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {importResult.imported} income receipt{importResult.imported !== 1 ? "s" : ""} totalling{" "}
