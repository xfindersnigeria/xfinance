...
import { useDeleteStatutoryDeduction } from "@/lib/api/hooks/useSettings";
...
export default function StatutoryDeductionActions({ row }: { row: StatutoryDeduction }) {
  const { isOpen, openModal, closeModal } = useModal();
  const deleteDeduction = useDeleteStatutoryDeduction();

  const editKey = `${MODAL.STATUTORY_DEDUCTION_EDIT}-${row.id}`;
  const deleteKey = `${MODAL.STATUTORY_DEDUCTION_DELETE}-${row.id}`;
  ...
  return (
    <>
      ...
      <CustomModal
        ...
        description="Update this mandatory deduction"
        open={isOpen(editKey)}
