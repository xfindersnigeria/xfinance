...
export function CreatePlanForm({
  ...
  const form = useForm<TierFormData>({
    ...
    defaultValues: {
      name: tier?.name || "",
      description: tier?.description || "",
      monthlyPrice: tier?.monthlyPrice !== undefined ? tier.monthlyPrice.toString() : "",
      yearlyPrice: tier?.yearlyPrice !== undefined ? tier.yearlyPrice.toString() : "",
      maxUsers: tier?.maxUsers !== undefined ? tier.maxUsers.toString() : "",
      maxEntities: tier?.maxEntities !== undefined ? tier.maxEntities.toString() : "",
      customBranding: tier?.customBranding ?? false,
      prioritySupport: tier?.prioritySupport ?? false,
      moduleIds: tier?.subscriptionModules?.map((m: any) => m.moduleId) || [],
    },
