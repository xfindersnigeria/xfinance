...
async function seedModules() {
  try {
    ...
    const modules = [
      ...
      {
        key: 'masterChartOfAccounts',
        name: 'Master Chart of Accounts',
        menu: 'Master Chart of Accounts',
        scope: 'admin',
        menuSortOrder: 4,
        moduleSortOrder: 0,
      },
    ...
    //   const moduleScope =
    //     module.scope.toUpperCase() === 'USER'
    //       ? ModuleScope.ENTITY
    //       : module.scope.toUpperCase() === 'SUPERADMIN'
    //         ? ModuleScope.SUPERADMIN
    //         : ModuleScope.GROUP;
    ...
    //         isOptional:
    //           module.isOptional && module.isOptional === true ? true : false,
    //         scope: moduleScope,
    ...
    //     });
    //     console.log(
    //       `✓ Created module: ${module.name} [${module.scope.toUpperCase()}]`,
    //     );
    //   } else {
      ...
      if (!existingModule) {
        ...
        await prisma.module.create({
          data: {
            ...
            menuSortOrder: module.menuSortOrder,
            moduleSortOrder: module.moduleSortOrder,
            isOptional: module.isOptional === true,
            scope: moduleScope,
            ...(module.scope !== 'superadmin' && {
              actions: {
                create: actions.map((action) => ({
        ...
        // ONLY UPDATE isOptional (nothing else touched)
        if (existingModule.isOptional !== (module.isOptional === true)) {
          ...
          console.log(`↻ Updated isOptional: ${module.name}`);
        } else {
          console.log(`✓ No change: ${module.name}`);
        }
      }
    }
